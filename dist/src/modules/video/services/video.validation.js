...
const videos_1 = require("../entity/videos");
class VideoValidator {
    static error(field, message) {
        return { field, message };
    }
    static isStringWithLength(value, maxLength) {
        return (typeof value === "string" &&
            value.trim().length > 0 &&
            value.length <= maxLength);
    }
    static validateStringField(value, field, maxLength) {
    ...
    static validatePublicationDate(date) {
        ...
        if (date && !isoDateRegex.test(date)) {
            return this.error("publicationDate", "publicationDate must be a valid ISO 8601 date");
        }
        return null;
    }
    static validateBooleanField(value, field) {
        if (value !== undefined && typeof value !== "boolean") {
            return this.error(field, `${field} must be a boolean`);
        }
        return null;
    }
    static validateArrayContainsValidResolutions(resolutions) {
        if (!Array.isArray(resolutions) ||
            resolutions.some((r) => !videos_1.AvailableResolution[r])) {
    ...
    static validateCreateVideoInput(videoDto) {
        return [
