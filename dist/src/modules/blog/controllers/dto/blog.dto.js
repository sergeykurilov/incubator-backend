"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BlogDto = void 0;
const class_validator_1 = require("class-validator");
class BlogDto {
}
exports.BlogDto = BlogDto;
__decorate([
    (0, class_validator_1.IsString)({ message: "Name must be a string" }),
    (0, class_validator_1.Length)(1, 15, { message: "Name must be between 1 and 15 characters" }),
    __metadata("design:type", String)
], BlogDto.prototype, "name", void 0);
__decorate([
    (0, class_validator_1.IsString)({ message: "Description must be a string" }),
    (0, class_validator_1.Length)(1, 500, {
        message: "Description must be between 1 and 500 characters",
    }),
    __metadata("design:type", String)
], BlogDto.prototype, "description", void 0);
__decorate([
    (0, class_validator_1.IsString)({ message: "Website URL must be a string" }),
    (0, class_validator_1.Length)(1, 100, {
        message: "Website URL must be between 1 and 100 characters",
    }),
    (0, class_validator_1.Matches)(/^https:\/\/([a-zA-Z0-9_-]+\.)+[a-zA-Z0-9_-]+(\/[a-zA-Z0-9_-]+)*\/?$/, { message: "Incorrect website URL" }),
    __metadata("design:type", String)
], BlogDto.prototype, "websiteUrl", void 0);
